...
- `P17` 已补第十三段真实 mc-agent 协议 smoke：按 `github_girl` `smoke_local.py` / `README.md` 的目标，新增 `scripts/smoke-minecraft-agent.mjs` 与 `npm run smoke:minecraft-agent`。它会连真实 `ws://localhost:48909`（可用 `MC_AGENT_WS` / `--ws` 改），发送 `query_inventory`，再发送带 `task_id` 的 `task` 帧，等待匹配的 `task_finished`，统计 `log/screenshot/inventory/agent_status/alert`，可用 `--dump-dir` 保存截图。适配点：`github_girl` 直接实例化 Python `GameAgentService` 并假装 push_message；本项目主逻辑绑定 Electron main/nativeImage，不适合在普通 Node 里直接实例化，所以先做 Node 版真实 WebSocket 协议 smoke，后续再补 Electron 内部服务级 smoke。

当前进度估算：整体迁移约 63%；核心桌宠/Live2D 体验约 73%；屏幕/摄像头视觉链路约 76%；Minecraft P17 当前项目内闭环约 99%，完整游戏 Agent 自主玩法约 71%。
