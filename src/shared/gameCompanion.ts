...
const MINECRAFT_INVENTORY_RE = /背包|物品栏|inventory|身上.*(?:有什么|有啥|多少|物品)|(?:查|看|看看).*(?:物品|资源)/i;
const MINECRAFT_STATUS_RE = /(?:状态|进度|任务|代理|agent).*(?:怎么样|怎样|如何|完成|连上|是什么)|(?:当前|现在).*(?:任务|状态|进度)/i;
const MINECRAFT_STOP_TASK_RE = /(?:停止|取消|中断|打断|停下|别做).*(?:任务|动作|操作|当前|挖|砍|找|走|做)?|(?:先停|停一下|别动了)/i;
const MINECRAFT_TASK_SIGNAL_RE =
  /帮我|替我|让(?:她|你|AI|ai|猫猫)?|叫(?:她|你)?|去|开始|继续|执行|挖|采|砍|找|建|造|收集|探索|跟|跟着|攻击|打|跑|回家|睡觉|吃|放|拿|制作|合成|整理|走|往|躲|避开|杀|防御|等待|守住|种|钓|交易|装备|熔炼|烧|mine|dig|chop|collect|find|build|craft|follow|attack|explore|return|go|stop|come/i;
...
const MINECRAFT_TASK_PRESETS: Array<[RegExp, string]> = [
  ...
  [/(跟着我|跟随我|跟我|follow)/i, 'follow the player closely and avoid getting stuck'],
...
export function buildGameCompanionPrompt(
  ...
  return [
    ...
    minecraftStatus?.pendingTask
      ...
      : '如果她现在空闲，可以主动挑下一步：要么说一句接下来想干什么，要么等待用户明确指令。不要站着挂机，也不要为了凑任务硬编动作。',
    `如果没有值得说的新东西，reply 必须严格等于 "${GAME_COMPANION_NO_COMMENT}"。`,
    '不要说“系统”“工具”“截图”“模型”“连接”“任务空闲”“minecraft_task”“tool”“我在观察屏幕”等内部状态；像坐在旁边一起玩的人。',
